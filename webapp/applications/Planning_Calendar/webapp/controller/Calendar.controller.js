...
function (Controller, JSONModel, MessageBox, Token, Filter, FilterOperator, formatter) {
	...
	return Controller.extend("MDG.Planning_Calendar.controller.Calendar", {
							...
							if (appProperty == checkAppoint[j].title) {
								...
								for (var u = 0; u < checkAppoint[j].users_permissions_users.data.length; u++) {
									...
									var change = {
										...
										firstName: checkAppoint[j].users_permissions_users.data[u].attributes.firstName,
										lastName: checkAppoint[j].users_permissions_users.data[u].attributes.lastName,
									}
									Set.push(change);
		...
		onSelectUserFilter: function (evt) {
			var that = this;
			var aContexts = evt.getParameter("selectedContexts");
			that.participants = [];
			that.userManagment = [];
			...
			that.AppointmentInfoFilter.getContent()[1].getModel().setData(mainObj);
			that.AppointmentInfoFilter.getContent()[1].getModel().updateBindings();
			...
			this.Selectuserfilter.getModel().updateBindings();
			//	that._oAppointmentInfo.getModel().getData().push(that.participants);
			...
			$.ajax({
				...
				success: function (res) {
					...
					if (getValues.error) {
						...
						that.getView().getModel("appointmentsdata").updateBindings();
						that.getView().getModel("appointmentsdata").refresh();
						MessageBox.success("Participant Updated Successfully ");
						// that.onCloseDetailPress();
						that.onInit();
						that.handleAppointmentSelect(oEvent);
						...
						$.ajax({
							...
							success: function (res) {
								...
								if (getValues.error) {
									...
									//	that.getView().getModel("appointmentsdata").updateBindings(true);
									// that.onCloseDetailPress();
									//	this.handleAppointmentClear();
									//	this.getView().getModel("appointmentsdata").updateBindings(true);
									//	this.handleAppointmentClear();
									...
									//	debugger
									//	this.getView().getModel("appointmentsdata").updateBindings(true);
									// this.getView().getModel("appointmentsdata").refresh();
									// this.AppointmentInfoFilter.close();
									// //this.handleAppointmentClear();
									// // this.getView().getModel("programsfilter").updateBindings(true);
									// // this.getView().getModel("programsfilter").refresh();
									// //	this.getView().getModel("prgData").updateBindings(true);
									// // MessageBox.success("Deleted Successfully");
									// //	this.getView().getModel("appointmentsdata").updateBindings(true);
									// this.onInit();
									// this.getView().getModel("programsfilter").refresh();
		...
		onSearchParticipantValueHelpFilter: function (evt) {
			this.Selectuserfilter.getBinding("items").filter([new sap.ui.model.Filter("firstName", sap.ui.model.FilterOperator.Contains,
				evt.getParameters().value)]);
		},
		...
		handleAddExternalParticipantsFilter: function (evt) {
			if (!this.Selectuserfilter) {
				this.Selectuserfilter = sap.ui.xmlfragment(this.getView().getId(), "MDG.Planning_Calendar.fragment.Selectuserfilter", this);
				this.Selectuserfilter.setModel(this.getOwnerComponent().getModel("i18nModel"), "i18n");
				this.getView().addDependent(this.Selectuserfilter);
			}
			let getData = this.userData.users_permissions_users == undefined ? [this.userData] : this.userData.users_permissions_users.data;
			var getDatal = this.participants;
			let userModel = this.getView().getModel("usersDetails").getData();
			this.Selectuserfilter.setModel(new sap.ui.model.json.JSONModel(userModel));
			let aList = this.Selectuserfilter._oList.getItems();
